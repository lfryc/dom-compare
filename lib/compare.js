   ...
   Comparator.prototype._compareAttributes = function(expected, actual) {
      ...
      for(i in aExpected) {
         ...
         if(aExpected.hasOwnProperty(i) && aActual.hasOwnProperty(i)) {
            ...
            if(this._options.ignoreSequenceIdDifference && i == 'id') {
               if (vExpected !== vActual) {
                 var numberSuffix = /[0-9]+$/;
                 if (vExpected.replace(numberSuffix, '') == vActual.replace(numberSuffix, '')) {
                   ignore = true;
                 }
               }
            }
            if(this._options.ignoreStyleDifference && i == 'style') {
              if (vExpected !== vActual) {
                ignore = true;
              }
            }
            if(!ignore && vExpected !== vActual) {
